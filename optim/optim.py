...
def train(model, cost, perf_mon, conf, out_path, exp_name, iter_eval, logging,
  ...
  model_out = model.fwd(*model.fwd_inputs)
  ...
  tmp_out = fwd_model(*valid_data[0])[0]
  print tmp_out.shape
  print valid_data[0][0].shape

  #cnt = 0
  #for i in xrange(tmp_out.shape[0]):
  #  for j in xrange(tmp_out.shape[1]):
  #    #for ii in xrange(tmp_out.shape[2]):
  #      #for jj in xrange(tmp_out.shape[3]):
  #    if np.isnan(tmp_out[i,j,ii,jj]) or np.isinf(tmp_out[i,j,ii,jj]):
  #      cnt += 1
  #print cnt
