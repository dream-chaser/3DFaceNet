...
class PerformanceMonitor(object):
  ...
  def eval_measures_czx(self, x, fwd, logging):
    total_num = len(x)
    right_num = 0
    for i in xrange(total_num):
      out = fwd(*x[i])[0]
      #print(out)
      tmp = np.where(out == np.max(out))[0]
      if tmp.shape[0] == 0:
        print 'Maybe nan error!'
        continue
      predict_label = tmp[0]
      true_label = np.where(x[i][1] == np.max(x[i][1]))[0][0]
      if(predict_label == true_label):
        right_num += 1
    accuracy = float(right_num) / total_num
    logging.info("Accuracy is %f", accuracy)
    return accuracy
